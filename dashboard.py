...
def control_traffic():
    ...
    # [4] 센서 데이터가 오래되면 자동 제어를 일시 중지
    if not _is_fresh():
        st.warning("센서 데이터가 오래되었습니다. 자동 제어를 일시 중지합니다.")
        return
    ...
    payload = {"type": "led", "status": desired}
    try:
        ser.write((json.dumps(payload) + "\n").encode())
        st.session_state.led_status = desired
        st.toast("LED를 켰습니다." if desired == "on" else "LED를 껐습니다.")
